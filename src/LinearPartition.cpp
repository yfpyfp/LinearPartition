...
void BeamCKYParser::prepare(unsigned len)
{
    ...
    unpaired_prob.resize(seq_length + 1, 1.0);
    AUP = 0.0;
}
...
double BeamCKYParser::parse(string &seq)
{
...
#else
    if (seq_length > 0)
        Fast_LogPlusEquals(bestC[0].alpha, score_external_unpaired(0, 0));
    if (seq_length > 1)
        Fast_LogPlusEquals(bestC[1].alpha, score_external_unpaired(0, 1));
#endif
    ...
    if (is_verbose)
        fprintf(stderr, "Partition Function Calculation Time: %.2f seconds.\n", parse_elapsed_time);
