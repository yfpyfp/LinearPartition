...
void BeamCKYParser::cal_AUP(string &seq)
{
    for (const auto &entry : Pij)
    {
        ...
        pf_type prob = entry.second;
        unpaired_prob[i] -= prob;
        unpaired_prob[j] -= prob;
    }

    for (int i = 1; i <= seq_length; i++)
    {
        if (unpaired_prob[i] < 0)
            unpaired_prob[i] = 0;
    }

    pf_type total_unpaired = 0.0;

    for (int i = 1; i <= seq_length; i++)
    {
        total_unpaired += unpaired_prob[i];
    }
...
void BeamCKYParser::outside(vector<int> next_pair[])
{
    ...
    if (is_verbose)
        fprintf(stderr, "Base Pairing Probabilities Calculation Time: %.2f seconds.\n", bpp_elapsed_time);
